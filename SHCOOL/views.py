...
def Search(request):
    if request.method == 'POST':
        search = CAREER.objects.filter(Q(Title = request.POST['search']) | Q(Description = request.POST['search']) | Q(Description__contains = request.POST['search']) | Q(Title__contains = request.POST['search']) | Q(Title__istartswith = request.POST['search']))
